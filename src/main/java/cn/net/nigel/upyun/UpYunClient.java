...
public class UpYunClient {
     ...
     * @return UpYun object
     */
    public UpYunClient(UpConfig config, String userName, String password) throws Exception {
        this.config = config;
    ...
    public HttpURLConnection listFilesConnection(String bucketName, String prefix, String marker, int limit)
        ...
        // 获取链接
        URL url = new URL("http://" + UpConfig.apiDomain + uri);
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setConnectTimeout(config.connect_timeout);
        conn.setReadTimeout(config.read_timeout);
        conn.setRequestMethod(UpConfig.METHOD_GET);
        conn.setUseCaches(false);
        String date = getGMTDate();
        conn.setRequestProperty(UpConfig.DATE, date);
        conn.setRequestProperty(UpConfig.AUTHORIZATION, UpYunUtils.sign(UpConfig.METHOD_GET, date, uri, userName, password, null));
        conn.setRequestProperty("x-list-iter", marker);
//        conn.setRequestProperty("x-list-order", "desc");
//        conn.setRequestProperty("Accept", "application/json");
    ...
    public static String md5(String str) throws Exception {
        ...
        md5.update(str.getBytes(UpConfig.UTF8));
